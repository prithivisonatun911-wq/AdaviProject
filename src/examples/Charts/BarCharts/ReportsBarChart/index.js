...
function ReportsBarChart({ color, title, description, chart, items }) {
  ...
  const renderItems = items.map(({ icon, label, progress }) => (
    <Grid item xs={6} sm={3} key={label}>
      <BarReportsChartItem color={color} label={label} icon={{ color: icon.color, component: icon.component }} progress={{ content: progress.content, percentage: progress.percentage }} />
    </Grid>
  ));
  ...
  return (
    <Card sx={{ height: "100%" }}>
      <ArgonBox padding="1rem">
        {useMemo(
          () => (
            <ArgonBox variant="gradient" bgColor={color} borderRadius="lg" py={2} pr={0.5} mb={3} height="12.5rem"            >

...
function mapAuthorsToTable(data, type) {

  console.log(data)
  console.log(type)

  if (type === "userDetail" && Array.isArray(data) && data.length > 0) {
    const { image, name, email } = data[0];
    return <Author image={image} name={name} email={email} />;
  }
  return null;
}
export default mapAuthorsToTable;*/
